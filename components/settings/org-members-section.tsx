...
const ROLE_COLORS: Record<OrgMember["role"], string> = {
  ...
  member: "bg-zinc-500/15 text-zinc-600 border-zinc-500/20",
};
...
export default function OrgMembersSection(props: Props) {
  return (
    <InviteMembersModalProvider>
      <OrgMembersSectionContent {...props} />
    </InviteMembersModalProvider>
  );
}

function OrgMembersSectionContent({ orgSlug, orgUserRole }: Props) {
  const { members, isLoading, removeMember, updateMemberRole } = useOrgMembers(orgSlug);
  const { open: openInvite } = useInviteMembersModal();
  const [targetMember, setTargetMember] = useState<OrgMember | null>(null);
  const [isRemoving, setIsRemoving] = useState(false);
  const [updatingRole, setUpdatingRole] = useState<string | null>(null);

  const isOwner = orgUserRole === "owner";
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      toast.success(`${displayName(targetMember)} removed from organisation`);
  ...
  const handleRoleChange = async (member: OrgMember, role: OrgMember["role"]) => {
    ...
    try {
      ...
      toast.success(`${displayName(member)}'s role updated to ${role}`);
    } catch (err) {
        ...
        <div className="divide-y divide-border rounded-md border">
          {members?.map((member) => (
            ...
            >
              ...
              <Badge
                className={`capitalize hidden sm:inline-flex ${ROLE_COLORS[member.role]}`}
                variant="outline"
              >
                {member.role}
              </Badge>
              {isOwner ? (
                <Select
                  ...
                  onChange={(e) =>
                    handleRoleChange(member, e.target.value as OrgMember["role"])
                  }
                >
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              this organisation? They will lose access to all workspaces.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button
